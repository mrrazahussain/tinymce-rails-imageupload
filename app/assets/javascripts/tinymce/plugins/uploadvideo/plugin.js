(function() {
  tinymce.PluginManager.requireLangPack('uploadvideo');

  tinymce.create('tinymce.plugins.UploadVideo', {
    UploadVideo: function(ed, url) {
      var form,
          iframe,
          win,
          throbber,
      ...
      function showDialog() {
        win = editor.windowManager.open({
          title: ed.translate('Insert an video file from your computer'),
          width:  500 + parseInt(editor.getLang('uploadvideo.delta_width', 0), 10),
          height: 180 + parseInt(editor.getLang('uploadvideo.delta_height', 0), 10),
          body: [
            {type: 'iframe',  url: 'javascript:void(0)'},
            {type: 'textbox', name: 'file', label: ed.translate('Choose an video file'), subtype: 'file'},
        ...
        form = createElement('form', {
          action: ed.getParam("uploadvideo_form_url", "/tinymce_assets"),
          target: iframe._id,
          method: "POST",
        ...
        form.appendChild(createElement('input', {type: 'hidden', name: 'hint', value: ed.getParam("uploadvideo_hint", "")}));
      ...
      function buildHTML(json) {
        var default_class = ed.getParam("uploadvideo_default_class", "");
        var figure = ed.getParam("uploadvideo_figure", false);
        var alt_text = getInputValue("alt");
        var imgstr = "<video controls='controls'";
        if(json["video"]["height"])
          imgstr += " height='" + json["video"]["height"] + "'";
        if(json["video"]["width"])
          imgstr += " width='"  + json["video"]["width"]  + "'";
        imgstr+='>';
        imgstr+= "<source src='" + json["video"]["url"] + "'";
        if(default_class != "")
      ...
      editor.addButton('uploadvideo', {
        tooltip: ed.translate('Insert an video file from your computer'),
        icon : 'media',
        onclick: showDialog
      });
      ...
      editor.addMenuItem('uploadvideo', {
        text: ed.translate('Insert an video file from your computer'),
        icon : 'media',
        context: 'insert',
        onclick: showDialog
      });
  ...
  tinymce.PluginManager.add('uploadvideo', tinymce.plugins.UploadVideo);
})();
